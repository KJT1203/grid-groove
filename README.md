...
**Instruments** — up to 8 tracks, each with its own instrument, octave, note length, level and TONE. Click cells in the note grid to cycle them off / on / accent — an accent is about 45% louder — and a column can hold as many notes as you like, so chords work. Notes are locked to the KEY and SCALE at the top, so nothing lands wrong. Notes belonging to the other tracks show as faint ghosts, so parts can be written against each other.
...
- **Songs are a string**: `v3|bpm|swing|key|scale|reverb|chain|kit|patterns|echo|drumkit|humanize`, with each pattern's notes packed as 12-bit hex masks and accents appended after an `=` only when a track actually uses them. That same string is the URL hash, the localStorage value and the undo history. Old `v1` and `v2` links still load.
